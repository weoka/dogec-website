 ...
 * @version 2.1
 * ---------------------
 * MOD by PDC: JSON+LD support, Cache in System Temp path, PHP7 compatible, Strip HTML comments
 * MOD by PDC: Alter script and style URLs to include versioning based on file size for use with "minify"
 * MOD by PDC: Remove spaces between tags, method="get" from forms
 */
...
$setting = array(
    'compress'            => true,
    'cache-folder'        => 'TEMP', // /cache
    'minify-css-files'    => true,
    'minify-js-files'     => true,
	'strip-html-comments' => true,
	'minify-url-path'     => 'minify/'
);
...
if ($setting['cache-folder'] == 'TEMP') {
	$cacheDir = sys_get_temp_dir() . '/dwccache';
}
else if ($setting['cache-folder'] == 'AUTO') {
    $cacheDir = dirname(__FILE__) . '/dwccache';
} else {
    $cacheDir = dirname(__FILE__) . $setting['cache-folder'];
}
...
function compressor_ob_handler($start_buffer)
{
	...
	$start_buffer = preg_replace('/<script\s*type="application\/ld\+json"\s*>/', '<json type="application/ld+json"> ', $start_buffer); // JSON
	...
	$start_buffer = str_replace('<json', '<script', $start_buffer);			// JSON
	$start_buffer = preg_replace('/"\s*\/>/', '"/>', $start_buffer);		// Spaces
	$start_buffer = preg_replace('/<br\s*\/>/', '<br/>', $start_buffer);	// Spaces
	$start_buffer = str_ireplace(' method="get"', '', $start_buffer);		// Form Method "GET"

    $start_buffer = clean_escapes($start_buffer);

	if ($setting['strip-html-comments'] == true)
		$start_buffer = preg_replace('/<!--(.|\s)*?-->/', '', $start_buffer);


	// Minify Versioning
	$strMinifyUrl = $setting['minify-url-path'];
	if ($strMinifyUrl != '') {
		$start_buffer = preg_replace_callback(
			...
			function($matches) {
				...
				$strMatch = $matches[1];
				$strMatch = preg_replace('/\s+/', ' ', $strMatch);
				$strMatch = trim(str_replace(' ', '', $strMatch));
				$strUrl   = $setting['minify-url-path'] . $strMatch;
				$intSize  = getUrlSize($strUrl);
				$strUrl  .= '?v=' . $intSize . '"';
...
function getUrlSize($strUrl) {
	if (!strpos($strUrl, 'http')) {
		if (substr($strUrl, 0, 1) == '/') {
			$strUrl = substr($strUrl, 1);
		}
		$strUrl = getHost() . '/' . $strUrl;
	}
	...
	if ($strData) {
		if (preg_match('/^HTTP\/1\.[01] (\d\d\d)/', $strData, $arrMatch)) {
			$intStatus = (int)$arrMatch[1];
		}

		if (preg_match('/Content-Length: (\d+)/', $strData, $arrMatch)) {
			$intLength = (int)$arrMatch[1];
		}

		if ($intStatus == 200 || ($intStatus > 300 && $intStatus <= 308)) {
			$intSize = $intLength;
		}
	}
