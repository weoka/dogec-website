...
/**
-------------------------------------------
PDC: Added i.ytimg.com to external domains
PDC: Added dynamic sharpening (0-100)
PDC: Prevent sharpening for PNG files
PDC: Added Tinify process before TimThumb
PDC: Changed cache expirations to 1 year
-------------------------------------------
...
if(! defined('TINIFY_ON') )					define ('TINIFY_ON', true);								// Enable Tinify lossless compression
if(! defined('TINIFY_LOCALHOST') )			define ('TINIFY_LOCALHOST', false);						// Run on dev/localhost environment?
...
if(! defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))	define ('FILE_CACHE_TIME_BETWEEN_CLEANS', 31536000);	// How often the cache is cleaned (86400)

if(! defined('FILE_CACHE_MAX_FILE_AGE') ) 	define ('FILE_CACHE_MAX_FILE_AGE', 31536000);				// How old does a file have to be to be deleted from the cache (86400)
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.tt.jpg');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'tt');				// What to put at the beg of all files in the cache directory so we can identify them
...
//Browser caching
if(! defined('BROWSER_CACHE_MAX_AGE') ) 	define ('BROWSER_CACHE_MAX_AGE', 31536000);				// Time to cache in the browser (864000)
...
class timthumb {
	...
	public function __construct(){
		...
		$strParamSrc = $this->param('src');
		$strParamSrc = str_replace('http/', 'http://', $strParamSrc);	// *** PDC MOD ***
		$strParamSrc = str_replace('https/', 'https://', $strParamSrc);	// *** PDC MOD ***
		$this->src = $strParamSrc;
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/tt_cacheLastCleanTime.touch';
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$sharpen = (int) $this->param('s', DEFAULT_S);
		...
		// sharpen image
		if (IMAGETYPE_PNG == $origType) $sharpen = 0;
		...
		/* Tinify (Lossless Compression) */
		// C:\inetpub\ePromoLogo2\cache\images\tim35C1.tmp
		// /home/epromologo/public_html/cache/images/timthumb_tmpimg_oNwGfN
		if (TINIFY_ON && ($_SERVER['HTTP_HOST'] != 'localhost' || TINIFY_LOCALHOST)) {
