<?php
/**
 * Name: Dynamic Website Search
 * Version: 3 [ MODS by Pablo Carrau ]
 * Thank you for purchasing me!
 * Author: David Baker
 * Website: http://tf.dtbaker.com.au/php-simple-website-search-tool/index.html
...
$GLOBALS['_SEARCH_HTML_DEPTH'] = 1;
$GLOBALS['_SEARCH_CACHE_LENGTH'] = 1; // days
...
$GLOBALS['_SEARCH_MIN_CHARS'] 			= 3;
$GLOBALS['_SEARCH_SUMMARY_LENGTH'] 		= 325;
...
$GLOBALS['_SEARCH_FILES_FOLDER'] 		= ""; // end it in a slash. path from search.php has to be writable
$GLOBALS['_SEARCH_CACHE_FOLDER'] 		= $GLOBALS['_SEARCH_FILES_FOLDER']."cache/search/"; // end it in a slash. path from search.php has to be writable
$GLOBALS['_SEARCH_REMOVE_HEADER'] 		= '<body>.*<div id="body_container">';
$GLOBALS['_SEARCH_REMOVE_FOOTER'] 		= '<div id="footer_container">.*</body>';
$GLOBALS['_SEARCH_ROOT'] 			    = true; // search by root or by relative path
//$GLOBALS['_SEARCH_FILE_HEADER'] 		= $GLOBALS['_SEARCH_FILES_FOLDER'].'inc_header.php'; // file that contains anything before search results
//$GLOBALS['_SEARCH_FILE_FOOTER'] 		= $GLOBALS['_SEARCH_FILES_FOLDER'].'inc_footer.php'; // file that contains anything after search results
...
function search_sample_callback($keyword,$search_depth){
	...
	return array(
		"something" => array(
			...
			"url" => "http://google.com/search?q=".urlencode($keyword), // Url to display / take the user to when they click on a search result.
		),
	);
}
?>
...
.phpsearch_list_item:hover{
	background-color: rgba(255, 250, 151, 0.2);
}
...
class website_search_dynamic{
	...
	public function __construct(){
		...
		if (!file_exists($GLOBALS['_SEARCH_CACHE_FOLDER']) && !is_dir($GLOBALS['_SEARCH_CACHE_FOLDER'])) {
			$this->createDir($GLOBALS['_SEARCH_CACHE_FOLDER']);
		}
			...
			if(isset($GLOBALS['_SEARCH_HTML_WEBSITES']) && is_array($GLOBALS['_SEARCH_HTML_WEBSITES'])){
				...
				foreach($GLOBALS['_SEARCH_HTML_WEBSITES'] as $website_url){
					...
					if($this->valid_url($website_url)){
						...
						if ($GLOBALS['_SEARCH_ROOT']) {
							$parts = parse_url($website_url);
							$this->valid_hosts[$parts['host']] = true;
							$website_url = $parts['scheme'] . '://' . $parts['host'] . '/';
						}
	...
	private function _crawl_url($url,$depth=0,$current_host='',$current_path = ''){
		...
		if(!$url_data){
			...
			if($url_data['html']){
				...
				$url_data['data'] = preg_replace('#<title>.*</title>#ismU','',$url_data['data']);
				$url_data['data'] = preg_replace('#' . $GLOBALS['_SEARCH_REMOVE_HEADER'] . '#ismU','',$url_data['data']);
				$url_data['data'] = preg_replace('#' . $GLOBALS['_SEARCH_REMOVE_FOOTER'] . '#ismU','',$url_data['data']);
	...
	public function render(){
		...
		if($GLOBALS['_SEARCH_SHOW_BOX']){
			...
			?>
			<form action="search?<?php echo $this->url_args;?>" method="get">
			<input type="text" name="q" class="phpsearch_input" size="35" autocomplete="off" value="<?php echo (isset($_REQUEST['q']))?htmlspecialchars($_REQUEST['q']):'';?>">
			<input type="submit" name="go" value="<?php echo $this->label('Search');?>" class="Btn1 phpsearch_button">
			</form>
			<p>&nbsp;</p>
			<?php
		}
		...
		if($this->searching && !$this->_fatal_error()){
			...
			<div class="phpsearch_pagination">
				...
				<a href="search?<?php echo $this->url_args;?>q=<?php echo urlencode($this->_original_search_term);?>&pg=<?php echo $current_page-1;?>" class="Btn1">&laquo;</a>
				<?php } ?>
				<span></span>
				<?php for($x=0;$x<$total_pages;$x++){
					$highlight_page = ($current_page == $x) ? 'Btn2' : 'Btn3';
					?>
					<a href="search?<?php echo $this->url_args;?>q=<?php echo urlencode($this->_original_search_term);?>&pg=<?php echo $x;?>" class="<?php echo $highlight_page;?>"><?php echo $x+1;?></a>
					<span></span>
					<?php }
				?>
				<?php if($current_page < ($total_pages-1)){ ?>
				<a href="search?<?php echo $this->url_args;?>q=<?php echo urlencode($this->_original_search_term);?>&pg=<?php echo $current_page+1;?>" class="Btn1">&raquo;</a>
				<?php } ?>
			</div>
			<?php
		}
	...
	public function createDir($strPath) {
		...
		foreach ($arrPath as $strDir) {
			if ($strDir != '') {
				$strFull .= $strDir . '/';
				if (!file_exists($strFull) && !is_dir($strFull)) {
					mkdir($strFull, 0777);
					$blnStat = true;
				}
			}
